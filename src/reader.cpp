...
void Reader::makeIdTaggs(string path_source, string path_target, string dest_source, string dest_target)
{
	//vector holding filenames
	stack<string> filenames;
	filenames = Utilities::findAllFiles(path_source);//filenames are the same in source and target directories
	...
	while(!filenames.empty())
	{
		...
		//tagg each file
		Reader::taggDocument(Utilities::findEuroparlFile(path_source,filename).c_str(), filename.c_str(), Utilities::findEuroparlFile(path_target,filename).c_str(), filename.c_str(), dest_source.c_str(), dest_target.c_str());
	}
}

//Numbers all Europarl documents
void Reader::numberData(string path_source, string path_target, string dest_source, string dest_target)
{
	...
	long int source_counter = 2;//numbering begins at 2 (1 : other_word, 0 : null_word)
...
//		cout << "Numbering " << filename << endl;
		...
		//tagg each file
		Reader::numberDocument(Utilities::findEuroparlFile(path_source,filename).c_str(), filename.c_str(), Utilities::findEuroparlFile(path_target,filename).c_str(), filename.c_str(), dest_source.c_str(), dest_target.c_str(), source_token_numbering, target_token_numbering, source_counter, target_counter);
	}
}


//Taggs document i.e. taggs sentences with their unique identifiers
void Reader::taggDocument(const char* path_untok_source, const char* filename_source, const char* path_untok_target, const char* filename_target, const char* dest_source, const char* dest_target)
{
	...
	else
	{
		...
		string combined_source = (prefix_source.append(name_source)).append(suffix_source);
			//make
		...
		while( getline ( fin1,line ) )
		{
			...
			//if a line is not a chapter tag, a sentence tag or a paragraph tag, it is a sentence
			if( !boost::regex_search( s, chapter_tag ) && !regex_search( s, speaker_tag ) && !regex_search( s, paragraph_tag ))
			{
				//Tag sentence
				sentence_counter++;
	...
	else
	{
		...
		string combined_target = (prefix_target.append(name_target)).append(suffix_target);
			//make output file
...
void Reader::numberDocument(const char* path_to_input_source, const char* filename_source, const char* path_to_input_target, const char* filename_target, const char* dest_source, const char* dest_target, map<string, long int> &source_token_numbering, map<string, long int> &target_token_numbering, long int &source_token_counter, long int &target_token_counter)
{
	//regex for chapter tags
	boost::regex chapter_tag( "<CHAPTER\\s+ID=\"?(\\d*)\"?>" );
	...
	else
	{
		...
		while( getline ( fin1,line ) )
		{
			//if a line is not a chapter tag, a speaker tag, a paragraph tag or a sentence tag, it is a sentence
			if( !boost::regex_search( line, chapter_tag ) && !regex_search( line, speaker_tag ) && !regex_search( line, paragraph_tag ) && ! regex_search(line, sentence_tag))
			{
...
stack<Data> Reader::splitEuroparlDocumentIntoParagraphs( const char * path1, const char * filename1,  const char * path2, const char * filename2)
{	
	//regex for chapter tags
	boost::regex chapter_tag( "<CHAPTER\\s+ID=\"?(\\d*)\"?>" );
	...
	bool same_speakers = sameEuroparlSpeakers(path1, path2);
	bool same_chapters = sameEuroparlChapters(path1, path2);
	bool same_paragraphs = sameEuroparlParagraphs(path1, path2);

	//check if document has the same number of paragraphs and if the paragraphs match
	if(same_chapters && same_speakers && same_paragraphs)
	{
		...
		if( !fin1 )
		{
			cout << "Couldn´t open input file : " << path1 << endl;
		}
		else
		{
			//look for tags to delimit paragraphs (chapter, speaker and paragraph tags are paragraph delimiters!)
			while( getline( fin1,line ))
			{
				//chapter tag is encountered
				if(boost::regex_search( line, chapter_tag ))
				{
					//if it appers after a sentence (not a speaker neither a paragraph tag)
				...
				{
					//if it appers after a sentence (not a chapter neither a paragrph tag)
		...
		else
		{
			...
			while( getline( fin2,line ))
			{
				...
				if(boost::regex_search( line, chapter_tag ))
				{
					//if it appers after a sentence (not a speaker neither a paragrph tag)
					if(!previous_is_speaker && !previous_is_paragraph && !is_first)
					{
						//read paragrph
						vector<Sentence> paragraph = readData(s);
	...
	else
	{
		...
		{
			cout << "No valid paragraph alignment. Document split at speaker delimiters" << endl;
			paragraphs = splitEuroparlDocumentIntoSpeakers( path1, filename1, path2, filename2);
		}
		//not the same number of paragraphs neihter the same number of speakers : split at chapter delimiters
		else if(same_chapters && !same_speakers)
		{
			cout << "No valid speaker alignment. Document split at chapter delimiters" << endl;
			paragraphs = splitEuroparlDocumentIntoChapters( path1, filename1, path2, filename2);
		}
		//not the same number of paragraphs neihter the same number of speakers neither the same number of chapters : no split possible
		else if(!same_chapters)
		{
			cout << "No valid chapter alignment. Alignment at document level." << endl;
...
stack<Data> Reader::splitEuroparlDocumentIntoSpeakers( const char * path1, const char * filename1,  const char * path2, const char * filename2)
{	
	//regex for chapter tags
	...
	else
	{
		...
		//read source file look for tags to delimit speakers (chapters are speaker delimiters!)
		while( getline( fin1,line ))
		{	
			//chapter tag is encountered
			if(regex_search( line, chapter_tag )) 
	...
	{
		//read target language file to delimit speaker paragraphs (chapter tags are speaker delimiters!)
		while( getline( fin2,line ))
		{	
			//chapter tag is encountered
			if(regex_search( line, chapter_tag )) 
...
stack<Data> Reader::splitEuroparlDocumentIntoChapters(const char * path1, const char * filename1,  const char * path2, const char * filename2)
{
	//regex for chapter tags
	regex chapter_tag( "<CHAPTER\\s+ID=\"?(\\d*)\"?>" );
	regex speaker_tag("^<SPEAKER\\s+ID=\"?(\\d*)\"?");
	...
	{	
		...
		while( getline( fin2,line ))
		{		
			//if a chapter tag is encontered put subsequent text until new delimiter into new paragraph 
			if(regex_search( line, chapter_tag )) 
			{
				if(!is_first)
				{
...
Data Reader::readNumberedEuroparlDocument( const char * filename_1, const char * filename_2 )
{
	//regex for chapter tags
	boost::regex chapter_tag( "<CHAPTER\\s+ID=\"?(\\d*)\"?>" );
	...
	else
	{
		...
		while( getline ( fin1,line ) )
		{		
			//if document is in Europal tagging format, each line contains either a chapter, speaker, paragraph a sentence tag or a sentence
	...
	for( itr_sst=source_sentence_tags.begin(); itr_sst!=source_sentence_tags.end(); itr_sst++ )
	{
	...
	{
		cout << "Error: Number of source sentences and sentence ids should be the same " << endl;
		cout << "Number of sentences " << source_sentences.size() << endl ;
		cout << "Number of sentence ids " << source_sentence_positions.size() << endl;
	...
	else	
	{
		...
		for( itr_ss = source_sentences.begin() , itr_ssp = source_sentence_positions.begin(); 
		itr_ss != source_sentences.end() && itr_ssp != source_sentence_positions.end();
		itr_ss++, itr_ssp++ )
		{
			...
			Sentence source_sentence_temp = Sentence(position,sentence.size(),count_words_in_sentence, int_token);
	...
	else
	{
		...
		while( getline ( fin2,line ) )
		{
			//if document is in Tagged Europal format, each line contains either a chapter, speaker, paragraph a sentence tag or a sentence
	...
	//extract sentence positions from opening tags 
	//append opening tags in string s
	for( itr_tst = target_sentence_tags.begin(); itr_tst != target_sentence_tags.end(); itr_tst++ )
	{
	...
	{
		cout << "Error: Number of target sentences and sentence ids should be the same " << endl;
		cout << "Number of sentences " << target_sentences.size() << endl ;
		cout << "Number of sentence ids " << target_sentence_positions.size() << endl;
		cout << "Return empty Data " << endl;
		Data empty_data;
		return empty_data;
	}
	...
	{
		//Extract information neceassary to construct sentence object
		for( itr_ts = target_sentences.begin() , itr_tsp = target_sentence_positions.begin(); 
		itr_ts != target_sentences.end() && itr_tsp != target_sentence_positions.end();
		itr_ts++, itr_tsp++ )
		{
			...
			Sentence target_sentence_temp = Sentence(position,sentence.size(),countWordsInSentence,int_token);
...
Data Reader::readEuroparlDocument( const char * filename_1, const char * filename_2 )
{
	//regex for chapter tags
	boost::regex chapter_tag( "<CHAPTER\\s+ID=\"?(\\d*)\"?>" );
	...
	else	
	{
		...
		itr_ss++, itr_ssp++ )
		{
			...
			//get and count token from sentences
			vector< long int> int_token; // empty ints vector to construct sentence object
			vector< string > token;
			int count_words_in_sentence = 0;
			//tokenize
			char * sentCstr;
	...
	//loop through position and sentence vectors, store sentence info and write log_files
	//nbr of sentence positions and nbr of sentences should be the same 
	if( target_sentences.size() != target_sentence_positions.size() )
	{
		cout << "Error: Number of target sentences and sentence ids should be the same " << endl;
...
void Reader::splitEuroparlDataIntoParagraphs( string source_path, string target_path )
{
	stack<string> filenames;
	
	//source and target files have the same, looking in one directory is sufficient to get filenames
	...
	while(!filenames.empty())
	{
		...
		Reader::splitEuroparlDocumentIntoParagraphs(Utilities::findEuroparlFile(source_path,filename).c_str(), filename.c_str(), Utilities::findEuroparlFile(target_path,filename).c_str(), filename.c_str());
	}
}
...
vector<Sentence> Reader::readData(const string& paragraph)
{
	...
	//extract sentences positions from opening tags 
	//append opening tags in string s
	for( itr_sent_tags=sentence_tags.begin(); itr_sent_tags!=sentence_tags.end(); itr_sent_tags++ )
	{
	...
	{
		cout << "Error: Number of sentences and sentence ids should be the same " << endl;
	...
	{
		//Extract information neceassary to construct sentence object
		for( itr_sent = sentences.begin() , itr_sent_pos = sentence_positions.begin(); 
		...
		itr_sent++, itr_sent_pos++ )
		{
			...
			//construct source sentence
			Sentence sentence_temp = Sentence(position,sentence.size(),count_words_in_sentence,token);
...
void Reader :: makeTrainingData(string &path_to_best_alignments, string &path_to_training_data)
{
	...
	while(!filenames.empty())
	{
		...
		if( !fin1 ) {cout << "Couldn´t open input file" << path << endl;}
		else
		{
			//extract source and target sentences
			while( getline ( fin1,line ) )
			{		
				//line is either source sentence tag, target sentence tag, bead tag or sentence
				...
				else if(boost::regex_search( line, bead_tag ))
				{
					...
					for(itr_sent = source_sentence.begin(); itr_sent != source_sentence.end(); itr_sent++)
					{
					...
					for(itr_sent = target_sentence.begin(); itr_sent != target_sentence.end(); itr_sent++)
					{
						string token = *itr_sent;
						fout1 << token << " ";
					}
...
//replaces words in training data with integers and makes numbering maps (where contiguous ints are put into correspondence with integers 
void Reader :: numberTrainingData(string &path_to_training_data, string &path_to_numbered_training_data)
{	
	//get files in directory
	stack<string> filenames;
	...
	while(!filenames.empty())
	{
		...
		else
		{
			...
			//extract source sentence number training data and make token numbering maps
			while( getline ( fin1,line ) )
			{
				counter++;
				...
				if(counter%2 == 1)//odd number : source sentence
				{
					while (t != NULL)
					{
						//if source token is not into numbering map, insert and print corresponding number into file
						if(source_token_numbering.find(atol(t)) == source_token_numbering.end())
						{
						...
						//if source token is into numbering map, print corresponding number into file
						else
				...
				{	
					while(t != NULL)
					{	
						//if target token is not into numbering map, insert and print corresponding number into file
						if(target_token_numbering.find(atol(t)) == target_token_numbering.end())
						{
						...
						//if target token is into numbering map, print corresponding number into file
						else
	...
	string path_to_source_token_numbering = "./numbering_maps/source_token_numbering.txt";
	string path_to_target_token_numbering = "./numbering_maps/target_token_numbering.txt";
	
	//write token numbering
	Writer::writeTokenNumbering(source_token_numbering, target_token_numbering, path_to_source_token_numbering, path_to_target_token_numbering);
...
pair<vector<long int>, vector<long int> > Reader :: filterOutRareWordsFromTrainingData(int &occurrence_limit, const string &path_to_training_data, const string &path_to_pruned_training_data)
{	
			...
			while( getline ( fin1,line ) )
			{
				...
				if(counter%2 == 1)//odd number : source sentence
				{
					while (t != NULL)
					{
						...
						if(frequency_of_source.find(atol(t)) != frequency_of_source.end())
						{
				...
				else //even number : target sentence
				{
					...
					{
						//if token is into frequency map, increment count
						if(frequency_of_target.find(atol(t)) != frequency_of_target.end())
						{
	...
	for(itr_map = frequency_of_source.begin(); itr_map != frequency_of_source.end(); itr_map++)
	{
	...
	for(itr_map = frequency_of_target.begin(); itr_map != frequency_of_target.end(); itr_map++)
	{
		pair<long int, int> freq_entry = *itr_map;
		long int tok = freq_entry.first;
		int freq = freq_entry.second;
	...
	while(!filenames.empty())
	{
		...
		if( !fin4 ) {cout << "Couldn´t open input file : " << path << endl;}
		else
		{
	...
	pair<vector<long int>, vector<long int> > rare_words(rare_source_token, rare_target_token);
	return rare_words;
}

//remove given lists of words from the data
void Reader :: filterOutRareWordsFromData(vector<long int> &rare_source_words, vector<long int> &rare_target_words, string &input_path_source, string &input_path_target, string &path_to_filtered_source, string &path_to_filtered_target)
{
	...
	while(!filenames.empty())
	{
		...
		if( !fin1 ) {cout << "Couldn´t open input file : " << filename << endl;}
		else
		{
					...
					while (t != NULL)
	  				{
						...
						if(binary_search(rare_source_words.begin(), rare_source_words.end(), atol(t)))
						{fout << other_word << " ";}
						else{fout << atol(t) << " ";}
						t=strtok_r(NULL," ", &sp);
	  				}
	...
	while(!filenames.empty())
	{
		...
		if( !fin2 ) {cout << "Couldn´t open input file : " << filename << endl;}
		else
		{
			//make output stream for filtered data
			...
			{		
				...
				else 
				{
					...
					while (t != NULL)
	  				{
						//look if token is in rare source words set
						if(binary_search(rare_target_words.begin(), rare_target_words.end(), atol(t)))
						{fout << other_word << " ";}
						else{fout << atol(t) << " ";}
						t=strtok_r(NULL," ", &sp);
	  				}
...
void Reader :: makeCoOccurrenceMap(string &path_to_training_data, string &path_to_co_occurrence_map)
{
	...
	//co-occurrence map first entry : target token, second entry source_token_occurrence_line : source token
			...
			while( getline ( fin1,line ) )
			{
				...
				if(counter%2 == 1)//odd number : source sentence
				{
					...
					while (t != NULL) //loop through source token
					{
						...
						if(source_token_occurrence_line.find(atol(t)) != source_token_occurrence_line.end())
						{source_token_occurrence_line.find(atol(t))-> second.push_back(line_counter);}//if yes add line to lines vector
				
						//else make new entry
						//put all source token into a set
						vector< long int > line_vector;
						line_vector.push_back(line_counter);
						pair<long int, vector<long int> > source_entry(atol(t), line_vector); 
				...
				{pair<long int, set<long int> > target_entry(line_counter, target_sentence);
	...
	for(itr_source_token = source_token_occurrence_line.begin(); itr_source_token != source_token_occurrence_line.end(); itr_source_token++)
	{
		...
		//for each line, find target token and make union of them (without duplicates)
		for(itr_lines = lines.begin(); itr_lines != lines.end(); itr_lines++)
		{
			long int line = *itr_lines;

			set<long int> target_sentence = target_token_occurrence_line.find(line)->second;
			set<long int> :: iterator itr_t;
		
			for(itr_t = target_sentence.begin(); itr_t != target_sentence.end(); itr_t++)
			{
		...
		//for debugging
		//cout << "Size of target sentences " << all_target_sentences.size() << endl;

		for(itr_sent = all_target_sentences.begin(); itr_sent != all_target_sentences.end(); itr_sent++)
		{	
			long int target_token = *itr_sent;	
				
			if(!(binary_search(itr_sent + 1 , all_target_sentences.end(), target_token))) //check if duplicates
			{
				target_sentences_without_duplicates.push_back(target_token);
			}
		}
		//make entry and insert into co-occurrence map
		pair<long int, vector<long int > > entry(source_token, target_sentences_without_duplicates);
...
int Reader::sameEuroparlParagraphs(const char * path1, const char * path2)
{
	...
	{
		//read target file look for tags to delimit paragraphs (chapter, speaker and paragraph tags are paragraph delimiters!)	
		while( getline( fin1,line ))
		{		
			//if a chapter tag is encontered put subsequent text until new delimiter into new paragraph 
			if(regex_search( line, chapter_tag ) || regex_search( line, speaker_tag) || regex_search(line, paragraph_tag)) 
			...
			{
				all_paragraphs_in_source_speaker.push_back(source_paragraphs_in_speaker);
	...
	else
	{
		...
		{		
			...
			if(regex_search(line, speaker_tag))
			{
				all_paragraphs_in_target_speaker.push_back(source_paragraphs_in_speaker);
	...
	//cout << "Number of source paragraphs " << source_paragraphs_counter << endl;
	//cout << "Number of target paragraphs " << target_paragraphs_counter << endl;

	if(source_taggs_counter != target_taggs_counter)
	{return 0;}

	else if(source_paragraphs_counter != target_paragraphs_counter || source_paragraphs_counter == 0 || target_paragraphs_counter == 0)
	{return 0;}
	else
	{
		if(all_paragraphs_in_source_speaker.size() != all_paragraphs_in_target_speaker.size())
		{return 0;}
		else
		{
			...
			for(	itr_source = all_paragraphs_in_source_speaker.begin(),
				...
				itr_source != all_paragraphs_in_source_speaker.end(), itr_target != all_paragraphs_in_target_speaker.end();
...
bool Reader::sameEuroparlSpeakers(const char * path1, const char * path2)
{
	...
	else
	{
		...
		while( getline( fin1,line ))
		{	
			//if a speaker tag is encontered put subsequent text until new delimiter into new paragraph 
	...
	//for debugging
	//cout << "Number of speaker tags in source document " << source_speaker_ids.size() << endl;
	//cout << "Number of speaker tags in target document " << target_speaker_ids.size() << endl;
	...
	{
		for(itr_source_ids = source_speaker_ids.begin(), itr_target_ids = target_speaker_ids.begin(); itr_source_ids != source_speaker_ids.end(), itr_target_ids != target_speaker_ids.end(); itr_source_ids++, itr_target_ids++)
		{
...
//regex for chapter tags
	...
	//for debugging
	//cout << "Number of chapter tags in source document " << source_chapter_ids.size() << endl;
	//cout << "Number of chapter tags in target document " << target_chapter_ids.size() << endl;
	...
	{
		for(itr_source_ids = source_chapter_ids.begin(), itr_target_ids = target_chapter_ids.begin(); itr_source_ids != source_chapter_ids.end(), itr_target_ids != target_chapter_ids.end(); itr_source_ids++, itr_target_ids++)
		{
			string source_id = *itr_source_ids;
			string target_id = *itr_target_ids;
