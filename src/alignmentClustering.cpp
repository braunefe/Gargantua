...
// compute bead priors (P_ak()) and lambda factor for successions of insertions
// and deletions
...
void AlignmentClustering::estimatePriors(stack<Chunk> a) {
  ...
  // store size of previous alignments (until 3 before current (for
  // 1-to-4/4-to-1 clusters)
  ...
  while (!alignment_temp.empty()) {
    ...
    if (pass_counter > 2) {
      merge31_source_size =
          current_source_size + previous_source_size + source_size_two_before;
      merge31_target_size =
          current_target_size + previous_target_size + target_size_two_before;
    }
    if (pass_counter > 3) {
      merge41_source_size = current_source_size + previous_source_size +
                            source_size_two_before + source_size_three_before;
      merge41_target_size = current_target_size + previous_target_size +
                            target_size_two_before + target_size_three_before;
    }

    // count blocs
    // count blocs of more than 1 insertion or deletion
    if (merge21_source_size == 0 ||
        merge21_target_size == 0)  // merge21 = current size + previous size
    {
      bloc_counter++;
    } else {
    ...
    // merger 2-1
    if ((merge21_source_size == 1 && merge21_target_size > 1) ||
        (merge21_target_size == 1 && merge21_source_size > 1)) {
      ...
      previous_candidate_21 = 0;
    }

    // merger 3-1
    if ((merge31_source_size == 1 && merge31_target_size > 2) ||
        (merge31_target_size == 1 && merge31_source_size > 2)) {
      if (previous_candidate_31 == 0) {
        candidates_31++;
      }
      previous_candidate_31++;
    } else {
      previous_candidate_41 = 0;
    }

    // merger 4-1
    if ((merge41_source_size == 1 && merge41_target_size > 3) ||
        (merge41_target_size == 1 && merge41_source_size > 3)) {
  ...
  double ratio =
      (candidates_21 + candidates_31 + candidates_41) / (double)total_bloc_size;

  // put weigths depending on ration between candidates for mergers and total
  // bloc size
  if (ratio > 5) {
    weight = 1.0;
  }
  ...
  total_candidates = candidates_21 + candidates_31 + candidates_41 +
                     candidates_10 + candidates_11;
  ...
  // prior11 and prior10 = "empty" prior i.e. prior probability that no
  // alignment occurrs
...
// distance function for making merging decisions (based on distance function as
// defined in Moore 2002 but with Viterbi model 1 instead of summing up over all
// alignments)
long double AlignmentClustering::mergeDistanceFunction(
    LengthDistributions &length_distributions,
    LexicalDistributions &lexical_distributions, vector<long int> st,
    vector<long int> tt, vector<int> ss, vector<int> ts) {
  int s1 = 0;
  if (ss.empty()) {
    s1 = 0;
  } else {
  ...
  if (ss.size() == 0)  // insertion
  {
    ...
    float relft = 0.0f;
    if (length_distributions.relative_frequencies_of_source.find(t1) !=
        length_distributions.relative_frequencies_of_source.end()) {
      relft =
          length_distributions.relative_frequencies_of_source.find(t1)->second;
    }
    // if raw relative frequency not found put to uniform
    else {
      relft = 1.0 / length_distributions.relative_frequencies_of_target.size();
    }
    ...
    for (itr_target_token = tt.begin(); itr_target_token != tt.end();
      ...
      if (lexical_distributions.unigram_frequencies_of_target.find(token) !=
        ...
        uf = 0.000001f;
      }  // very low probability for words that have not been found
      wordDist += -log(uf);
    }
  ...
  else if (ts.size() == 0)  // deletion
  {
    // compute length statistics
    float relfs = 0.0f;
    if (length_distributions.relative_frequencies_of_source.find(s1) !=
        length_distributions.relative_frequencies_of_source.end()) {
      relfs =
          length_distributions.relative_frequencies_of_source.find(s1)->second;
    }
    // if raw relative frequency not found put to uniform
    else {
      relfs = 1.0 / length_distributions.relative_frequencies_of_source.size();
    }
    ...
    if (ss.size() == 1 && ts.size() == 1) {
      ...
      if (length_distributions.relative_frequencies_of_source.find(s1) !=
          length_distributions.relative_frequencies_of_source.end()) {
        relfs = length_distributions.relative_frequencies_of_source.find(s1)
                    ->second;
      }
      // if raw relative frequency not found put to uniform
      else {
        relfs =
            1.0 / length_distributions.relative_frequencies_of_source.size();
      }
    ...
    // compute length statistics for merger 1-to-3
      ...
      float distance =
          length_distributions.distanceMeasurePoiss(s1, t1 + t2 + t3);
      float relfs = 0.0f;
      if (length_distributions.relative_frequencies_of_source.find(s1) !=
          length_distributions.relative_frequencies_of_source.end()) {
        relfs = length_distributions.relative_frequencies_of_source.find(s1)
    ...
    // compute length statistics for merger 1-to-4
      ...
      float distance =
          length_distributions.distanceMeasurePoiss(s1, t1 + t2 + t3 + t4);
      float relfs = 0.0f;
      if (length_distributions.relative_frequencies_of_source.find(s1) !=
          length_distributions.relative_frequencies_of_source.end()) {
        relfs = length_distributions.relative_frequencies_of_source.find(s1)
    ...
    // compute length statistics for merger 1-to-5
      ...
      if (length_distributions.relative_frequencies_of_source.find(s1) !=
          length_distributions.relative_frequencies_of_source.end()) {
        relfs1 = length_distributions.relative_frequencies_of_source.find(s1)
                     ->second;
      }
      // if not found put to uniform
      else {
      ...
      if (length_distributions.relative_frequencies_of_source.find(s2) !=
          length_distributions.relative_frequencies_of_source.end()) {
        relfs2 = length_distributions.relative_frequencies_of_source.find(s2)
                     ->second;
      }
      // if not found put to uniform
      else {
    ...
    // compute length statistics for merger 3-to-1
      ...
      float distance =
          length_distributions.distanceMeasurePoiss(s1 + s2 + s3, t1);
      ...
      if (length_distributions.relative_frequencies_of_source.find(s3) !=
          length_distributions.relative_frequencies_of_source.end()) {
        relfs3 = length_distributions.relative_frequencies_of_source.find(s3)
                     ->second;
      } else {
        // if not found put to uniform
        relfs3 =
      ...
      lengthDist =
          distance - log(relfs1) - log(relfs2) - log(relfs3) - log(prior13_31);
    }
    // compute length statistics for merger 4 to 1
      ...
      long double distance =
          length_distributions.distanceMeasurePoiss(s1 + s2 + s3 + s4, t1);
      ...
      if (length_distributions.relative_frequencies_of_source.find(s4) !=
          length_distributions.relative_frequencies_of_source.end()) {
        relfs4 = length_distributions.relative_frequencies_of_source.find(s4)
                     ->second;
      } else {
        // if not found put to uniform
        relfs4 =
      ...
      lengthDist = distance - log(relfs1) - log(relfs2) - log(relfs3) -
                   log(relfs4) - log(prior14_41);
    }
    ...
    for (itr_target_token = tt.begin(); itr_target_token != tt.end();
      ...
      for (itr_source_token = st.begin(); itr_source_token != st.end();
        ...
        // If yes, get proba;
        if (lexical_distributions.translation_table.find(stok) !=
            lexical_distributions.translation_table.end()) {
          if (lexical_distributions.translation_table.find(stok)
                  ->second.find(ttok) !=
              lexical_distributions.translation_table.find(stok)
                  ->second.end()) {
            transProb = -log(lexical_distributions.translation_table.find(stok)
                                 ->second.find(ttok)
                                 ->second);
            if (transProb <= maxTP) {
              maxTP = transProb;
          ...
          // else proba is taken from null
            ...
            if (lexical_distributions.translation_table.find(0)
                    ->second.find(ttok) !=
              ...
              transProb = -log(0.0000001f);  // very low probability for words
...
void AlignmentClustering::alignmentClustering(stack<Chunk> a,
                                              LengthDistributions &l_d,
                                              LexicalDistributions &lex_d) {
  stack<Chunk> alignment_temp = a;  // stack used for precomputation of
                                    // distances
  ...
  stack<Chunk>
      reverse_alignment_1;  // reverse alignment for distance computation
  stack<Chunk> reverse_alignment_2;  // reverse alignment for building clustered
                                     // alignment
  ...
  // map containing precomputed distances (long double) and corresponding labels
  // (int)
  ...
  while (!reverse_alignment_1.empty()) {
    ...
    // compute and compare distance functions as in the dynamic programming
    // framework
    if ((merge21_source_size == 1 && merge21_target_size > 1) ||
        (merge21_target_size == 1 && merge21_source_size > 1)) {
      ...
      vector<vector<long int> > first_source_chunk_token =
          first_chunk.getSourceChunkToken();
      vector<vector<long int> > first_target_chunk_token =
          first_chunk.getTargetChunkToken();

      vector<vector<long int> > second_source_chunk_token =
          second_chunk.getSourceChunkToken();
      vector<vector<long int> > second_target_chunk_token =
          second_chunk.getTargetChunkToken();
      ...
      for (itr_chunk_tok = first_source_chunk_token.begin();
           itr_chunk_tok != first_source_chunk_token.end(); itr_chunk_tok++) {
      ...
      for (itr_chunk_tok = second_source_chunk_token.begin();
           itr_chunk_tok != second_source_chunk_token.end(); itr_chunk_tok++) {
        vector<long int> token = *itr_chunk_tok;
        vector<long int>::iterator itr_tok;
      ...
      for (itr_chunk_tok = first_target_chunk_token.begin();
           itr_chunk_tok != first_target_chunk_token.end(); itr_chunk_tok++) {
        vector<long int> token = *itr_chunk_tok;
        vector<long int>::iterator itr_tok;
      ...
      for (itr_chunk_tok = second_target_chunk_token.begin();
           itr_chunk_tok != second_target_chunk_token.end(); itr_chunk_tok++) {
        vector<long int> token = *itr_chunk_tok;
        vector<long int>::iterator itr_tok;
        merged_target_size.push_back(token.size());
      ...
      long double distance_of_previous_chunk = mergeDistanceFunction(
          l_d, lex_d, first_source_token, first_target_token, first_source_size,
          first_target_size);

      // distance of current
      long double distance_of_current_chunk = mergeDistanceFunction(
          l_d, lex_d, second_source_token, second_target_token,
          second_source_size, second_target_size);

      // distance of merged
      distance_of_merged = mergeDistanceFunction(
          l_d, lex_d, merged_source_token, merged_target_token,
          merged_source_size, merged_target_size);
    ...
    if ((merge31_source_size == 1 && merge31_target_size > 2) ||
      ...
      vector<vector<long int> > third_source_chunk_token =
          third_chunk.getSourceChunkToken();
      vector<vector<long int> > third_target_chunk_token =
          third_chunk.getTargetChunkToken();
      ...
      for (itr_chunk_tok = third_source_chunk_token.begin();
           itr_chunk_tok != third_source_chunk_token.end(); itr_chunk_tok++) {
        vector<long int> token = *itr_chunk_tok;
        vector<long int>::iterator itr_tok;
        merged_source_size.push_back(token.size());
      ...
      for (itr_chunk_tok = third_target_chunk_token.begin();
           itr_chunk_tok != third_target_chunk_token.end(); itr_chunk_tok++) {
        vector<long int> token = *itr_chunk_tok;
        vector<long int>::iterator itr_tok;
        merged_target_size.push_back(token.size());
    ...
    if ((merge41_source_size == 1 && merge41_target_size > 3) ||
      ...
      vector<vector<long int> > fourth_source_chunk_token =
          fourth_chunk.getSourceChunkToken();
      vector<vector<long int> > fourth_target_chunk_token =
          fourth_chunk.getTargetChunkToken();
      ...
      for (itr_chunk_tok = fourth_source_chunk_token.begin();
           itr_chunk_tok != fourth_source_chunk_token.end(); itr_chunk_tok++) {
        vector<long int> token = *itr_chunk_tok;
        vector<long int>::iterator itr_tok;
        merged_source_size.push_back(token.size());
      ...
      for (itr_chunk_tok = fourth_target_chunk_token.begin();
           itr_chunk_tok != fourth_target_chunk_token.end(); itr_chunk_tok++) {
        vector<long int> token = *itr_chunk_tok;
        vector<long int>::iterator itr_tok;
        merged_target_size.push_back(token.size());
  ...
  for (int i = 0; i <= (int)a.size();
       i++)  // loop trough alignment outputed by initialAlignment.cpp
  {
    if (i == 0)  // construct initial cell (root of traceback tree)
    {
      // make fist cell of first column
      pair<int, CellPtr> first(i,
                               CellPtr(new Cell(null_distance, null_previous)));
      ...
      pair<int, CellPtr> diag_cell(
          i, CellPtr(new Cell(max_distance, null_previous)));
      ...
      if (distances.find(label_of_distance) != distances.end()) {
        ...
        distance_1 = previousD;
      }  // if current distance is not found => no candidate for merge =>
         // distance of no merge = 0
      ...
      if (i > 1)  // merge 2-1
      {
        ...
        if (distances.find(label_of_distance) != distances.end()) {
          ...
          distance_2 = MAXD;
        }  // if current distance not found => no candidate for merge =>
           // distance of merge = infinity
      }
  ...
  while (!(current_cell.first == root_cell.first &&
    ...
    if (distanceMatrix[current_cell.first] != NULL) {
      ...
      cout << "No path has been found in DP procedure. Return empty alignment."
           << endl;
  ...
  while (!reverse_alignment_2.empty()) {
    ...
    // merge into 1-2/2-1
      ...
      Chunk merged_chunk(merged_source_chunk_position,
                         merged_target_chunk_position,
                         merged_source_chunk_token, merged_target_chunk_token);

      vector<long int> first_source_chunk_position =
          first_alignment.at(first_position).getSourceChunkPosition();
      vector<long int> first_target_chunk_position =
          first_alignment.at(first_position).getTargetChunkPosition();
      vector<long int> second_source_chunk_position =
          first_alignment.at(second_position).getSourceChunkPosition();
      vector<long int> second_target_chunk_position =
          first_alignment.at(second_position).getTargetChunkPosition();

      vector<long int>::iterator itr_chunk_pos;

      // put merged positions in merged chunk
      for (itr_chunk_pos = first_source_chunk_position.begin();
           itr_chunk_pos != first_source_chunk_position.end();
           itr_chunk_pos++) {
        long int position = *itr_chunk_pos;
        if (position != -1)  // deletions and insertions are marked by -1, if
                             // merging do not add the empty position -1
        {
          merged_chunk.addToSourceChunkPosition(position);
        }
      }

      for (itr_chunk_pos = second_source_chunk_position.begin();
           itr_chunk_pos != second_source_chunk_position.end();
           itr_chunk_pos++) {
        long int position = *itr_chunk_pos;
        if (position != -1)  // deletions and insertions are marked by -1, if
                             // merging do not add the empty position -1
      ...
      for (itr_chunk_pos = first_target_chunk_position.begin();
           itr_chunk_pos != first_target_chunk_position.end();
           itr_chunk_pos++) {
        long int position = *itr_chunk_pos;
        if (position != -1)  // deletions and insertions are marked by -1, if
                             // merging do not add the empty position -1
      ...
      for (itr_chunk_pos = second_target_chunk_position.begin();
           itr_chunk_pos != second_target_chunk_position.end();
           itr_chunk_pos++) {
        long int position = *itr_chunk_pos;
        if (position != -1)  // deletions and insertions are marked by -1, if
                             // merging do not add the empty position -1
    ...
    // merge into 1-3/3-1
      ...
      vector<long int> third_source_chunk_position =
          first_alignment.at(third_position).getSourceChunkPosition();
      vector<long int> third_target_chunk_position =
          first_alignment.at(third_position).getTargetChunkPosition();
      ...
      for (itr_chunk_pos = third_source_chunk_position.begin();
           itr_chunk_pos != third_source_chunk_position.end();
           itr_chunk_pos++) {
        long int position = *itr_chunk_pos;
        if (position != -1)  // deletions and insertions are marked by -1, if
                             // merging do not add the empty position -1
      ...
      for (itr_chunk_pos = third_target_chunk_position.begin();
           itr_chunk_pos != third_target_chunk_position.end();
           itr_chunk_pos++) {
        long int position = *itr_chunk_pos;
        if (position != -1)  // deletions and insertions are marked by -1, if
                             // merging do not add the empty position -1
    ...
    // merge into 1-to-4/4-to-1
      ...
      vector<long int> fourth_source_chunk_position =
          first_alignment.at(fourth_position).getSourceChunkPosition();
      vector<long int> fourth_target_chunk_position =
          first_alignment.at(fourth_position).getTargetChunkPosition();
      ...
      for (itr_chunk_pos = fourth_source_chunk_position.begin();
           itr_chunk_pos != fourth_source_chunk_position.end();
           itr_chunk_pos++) {
        long int position = *itr_chunk_pos;
        if (position != -1)  // deletions and insertions are marked by -1, if
                             // merging do not add the empty position -1
      ...
      for (itr_chunk_pos = fourth_target_chunk_position.begin();
           itr_chunk_pos != fourth_target_chunk_position.end();
           itr_chunk_pos++) {
        long int position = *itr_chunk_pos;
        if (position != -1)  // deletions and insertions are marked by -1, if
                             // merging do not add the empty position -1
