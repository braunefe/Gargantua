...
LexicalDistributions :: LexicalDistributions(const WordProbs& l_u_f_s, const WordProbs& l_u_f_t, const LexProbs& t_t)
{
...
//training of model-1 : fractional counts for word translation pairs in a sentence that are not bigger than obtained by random choice are not added to the count for that translation pair (see Moore 2002)
float LexicalDistributions :: trainModel1(string &path_to_co_occurrence_map, string &path_to_training_data, string &path_to_t_table, int nbr_iterations)
{
	...
	if( !fin1 ) {cout << "Cannot open Co-Occurrence map" << endl; abort();}

	//null word is first line of co-occurrence map; null word co-occurrs with all target token
	getline ( fin1,line );
	...
	if( !fin2 ) {cout << "Cannot open co-occurrence map" << endl; abort();}
	...
	while(!filenames.empty())
	{
		...
		if( !fin3 ) {cout << "Couldn´t open input file" << path << endl;}
		else{
			...
			while( getline ( fin3,line ) )
			{
				...
				for(itr_target_sent = target_chunk.begin();  itr_target_sent != target_chunk.end(); itr_target_sent++)
				{
					long int target_token = *itr_target_sent;
					total_proba_target.at(target_token) = 0.0;

					for(itr_source_sent = source_chunk.begin(); itr_source_sent != source_chunk.end(); itr_source_sent++) 
					{
						long int source_token = *itr_source_sent;
						t_prob += t_table.at(source_token).find(target_token)->second;
					
					}
					//put proba into slot for target token (only if not null (after pruning))
					if(t_prob != 0.0)
					{
				...
				for(itr_target_sent = target_chunk.begin();  itr_target_sent != target_chunk.end(); itr_target_sent++)
				{
					...
					{
						long int source_token = *itr_source_sent;
						t_prob = t_table.at(source_token).find(target_token)->second/total_proba_target.at(target_token);

						//put proba into count table
						//check if token is already in count table
						if(c_table.at(source_token).find(target_token) !=  c_table.at(source_token).end())
						{//if yes, add proba
						
							c_table.at(source_token)[target_token] += t_prob;
						}
						else
						{//add target token to submap
	...
	if( !fin6 ) {cout << "Cannot open Co-Occurrence map." << path_to_co_occurrence_map << endl; abort();}
	...
	{		
		...
		while(t != NULL)
		{
			...
			//if token is in count table, make t-proba by dividing by proba for all target token
			if(c_table.at(source_token).find(target_token) != c_table.at(source_token).end())
			{
				t_prob = c_table.at(source_token).find(target_token)->second/total_proba_source.at(source_token);
	...
	for(int c=1; c < nbr_iterations; c++)
	{
		...
		while(!filenames.empty())
		{
			...
			if( !fin1 ) {cout << "Couldn´t open input file" << path << endl; abort();}
			else{
				//go through file and put source and target sentences into maps
				while( getline ( fin1,line ) )
				{
					...
					for(itr_target_sent = target_chunk.begin();  itr_target_sent != target_chunk.end(); itr_target_sent++)
					{
						...
						for(itr_source_sent = source_chunk.begin(); itr_source_sent != source_chunk.end(); itr_source_sent++) 
						{
							...
							if(t_table.at(source_token).find(target_token) != t_table.at(source_token).end())
							{
								t_prob += t_table.at(source_token).find(target_token)->second;
								//for debugging
					...
					for(itr_target_sent = target_chunk.begin();  itr_target_sent != target_chunk.end(); itr_target_sent++)
					{
						...
						for(itr_source_sent = source_chunk.begin(); itr_source_sent != source_chunk.end(); itr_source_sent++) 
						{
							...
							//check if proba is big enough (if smaller than random do not add)
							...
							{
								if(c_table.at(0).find(target_token) !=  c_table.at(0).end())
								{//if yes, add proba
...
//renumbers t-table according to numbering maps from training data and writes new table into file
void LexicalDistributions :: renumberTTable(string &path_to_source_token_numbering, string &path_to_target_token_numbering, string &path_to_t_table, string &path_to_renumbered_table)
{
	...
	if( !fin1 ) {cout << "Cannot open source token numbering map." << path_to_source_token_numbering << endl; abort();}
	...
	while(getline( fin1,line ))
	{
		...
		if(strcmp(t,sep_1.c_str()) == 0)
		{
			t=strtok_r(NULL," ", &sp);//insert null and other words into source token
		}
		...
		pair<long int, long int> source_numbering_entry(numbered_token, original_token);
	...
	if( !fin2 ) {cout << "Cannot open source token numbering map." << path_to_target_token_numbering << endl; abort();}
	...
	while(getline( fin2,line ))
	{
		...
		pair<long int, long int> target_numbering_entry(numbered_token, original_token);
	...
	if( !fin3 ) {cout << "Cannot open T-Table" << endl; abort();}
	...
	while(getline( fin3,line ))
	{
		...
		if(source_token_numbering.find(source_token_id) != source_token_numbering.end())
		{
			long int source_token = source_token_numbering.find(source_token_id)->second;
			fout << source_token << " ";
		}
		else{cout << "Source token not found" << endl;}	

		//replace target token id by original id
		if(target_token_numbering.find(target_token_id) != target_token_numbering.end())
		{
			long int target_token = target_token_numbering.find(target_token_id)->second;
			fout << target_token << " ";
		}
		else{cout << "Target token not found" << endl;}		
...
void LexicalDistributions::computeUnigramFrequencies(string &input_path_source, string &input_path_target, string &path_to_unigram_frequencies)
{
	...
	while(!filenames.empty())
	{
		...
		Data d = Reader::readNumberedEuroparlDocument( Utilities::findEuroparlFile(input_path_source, filename).c_str(), Utilities::findEuroparlFile(input_path_target, filename).c_str());
		...
		for(itr_sentence=source_sentences.begin(); itr_sentence!= source_sentences.end(); itr_sentence++)
		{ 	
		...
		for(itr_sentence=target_sentences.begin(); itr_sentence!= target_sentences.end(); itr_sentence++)
		{ 	
			Sentence sent = *itr_sentence;
			vector<long int> tokens = sent.getTokenInts();
	...
	{	
		...
		while((binary_search(itr_token, source_token.end(), source_tok))) //check if duplicates
		{
			occurrence_counter++;
			itr_token++;
		}
	
		pair<long int, float> source_occurrence_entry(source_tok, occurrence_counter);
	...
	{	
		...
		float occurrence_counter = 0.0;	
		while((binary_search(itr_token, target_token.end(), target_tok))) //check if duplicates
		{
			occurrence_counter++;
			itr_token++;
		}
	
		pair<long int, float> target_occurrence_entry(target_tok, occurrence_counter);
		unigram_freq_of_target.insert(target_occurrence_entry);
	}

	//For debugging
	//cout << "Source and target frequency maps done " << endl;
	//cout << "Size of source frequency map " << unigram_freq_of_source.size() << endl;
	//cout << "Size of target frequency map " << unigram_freq_of_target.size() << endl;
	
	//compute number of source and target token to compute unigram frequency of "other word"
	...
	//loop through source frequency map collect source words that occurr the same number of times 
	boost::unordered_map<float, set<long int> > words_for_source_occurrence_count;

	for(itr_map = unigram_freq_of_source.begin(); itr_map != unigram_freq_of_source.end(); itr_map++)
	{
	...
	//loop through target frequency map collect target words that occurr the same number of times 
	boost::unordered_map<float, set<long int> > words_for_target_occurrence_count;

	for(itr_map = unigram_freq_of_target.begin(); itr_map != unigram_freq_of_target.end(); itr_map++)
	{
		pair<long int, float> entry = *itr_map;
	...
	//fix unigram frequency of "other token" to 1/number of source or target words
	...
	Writer::writeUnigramFrequencies(unigram_freq_of_source, unigram_freq_of_target, path_to_unigram_frequencies);
}


//prepares a t-table for each document by filtering out words that are in the document (document = numbered_data_stack)
void LexicalDistributions :: prepareTTable(stack<Data> numbered_data_stack, LexProbs &t_table)
{	
	...
	for(itr_map = t_table.begin(); itr_map != t_table.end(); itr_map++)
	{
		...
		for(itr_submap = entry.second.begin(); itr_submap != entry.second.end(); itr_submap++)
		{
	...
	//string path_to_prepared_tables = "./lexical_distributions/prepared_t_tables/";
	//string table_name = "current_table.txt";
	//Writer::writePreparedTTable(prepared_t_table, path_to_prepared_tables, table_name);
}

//prepares unigram frequencies for each document by filtering out words that are in the document (document = numbered_data_stack)
void LexicalDistributions :: prepareUnigramFrequencies(stack<Data> numbered_data_stack, string &path_to_unigram_frequencies_of_source, string &path_to_unigram_frequencies_of_target)
{
	WordProbs unigram_freq_of_source;
	WordProbs unigram_freq_of_target;
	...
	if( !fin1 ) {cout << "Cannot open Unigram frequencies of source at : " << path_to_unigram_frequencies_of_source << endl; abort();}
	...
	if( !fin2 ) {cout << "Cannot open Unigram frequencies of target at : " << path_to_unigram_frequencies_of_target << endl; abort();}
	...
	//for debugging
	//string path_to_unigram_frequencies = "./lexical_distributions/unigram_language_model/prepared_unigram_frequencies/";
	//Writer::writeUnigramFrequencies(unigram_freq_of_source, unigram_freq_of_target, path_to_unigram_frequencies);
}
...
float LexicalDistributions ::getTTableEntry(const long int stok, const long int ttok) const {
...
vector<float> LexicalDistributions::getTTableEntries(const long int stok, const vector<long int>& ttoks) const {
	vector<float> res;
