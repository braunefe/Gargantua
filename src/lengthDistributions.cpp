...
LengthDistributions::LengthDistributions(float param_of_poisson,
                                         map<int, float> rfs,
                                         map<int, float> rft) {
...
void LengthDistributions::setParamOfPoisson(float p) { param_of_poisson = p; }
...
void LengthDistributions::estimateParamOfPoisson(string input_path_source,
  ...
  while (!filenames.empty()) {
    ...
    Data my_data = Reader::readNumberedEuroparlDocument(
        Utilities::findEuroparlFile(input_path_source, file_name).c_str(),
        Utilities::findEuroparlFile(input_path_target, file_name).c_str());
  ...
  // for debugging
  // cout << "Number of sentences in source document " <<
  // number_of_source_sentences << endl;
  // cout << "Number of sentences in target document " <<
  // number_of_target_sentences << endl;
...
void LengthDistributions::computeRawRelativeFrequencies(
  ...
  while (!filenames.empty()) {
    ...
    for (itr_source_freq_map = freq_of_source.begin();
         itr_source_freq_map != freq_of_source.end(); itr_source_freq_map++) {
    ...
    for (itr_target_freq_map = freq_of_target.begin();
         itr_target_freq_map != freq_of_target.end(); itr_target_freq_map++) {
...
// computes poisson log probability for source and target sentence lenghts l1
// and l2
float LengthDistributions::distanceMeasurePoiss(int l1, int l2) const {
  ...
  // compute the mean of poisson : ls * r (with r= mean of lengths of sentences
  // in source/mean of length of sentences in target)
...
void LengthDistributions::writeRelativeFrequencies()
{
        ...
        ofstream
fout("length_alignment_log_files/raw_relative_frequencies.txt");
