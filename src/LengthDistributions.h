...
class LengthDistributions {
 ...
 public:
  ...
  LengthDistributions(float param_of_poisson, map<int, float> rfs,
                      map<int, float> rft);
  ...
  void estimateParamOfPoisson(string input_path_source,
                              string input_path_target);

  // Computes the raw relative frequencies of source and target sentence lengths
  void computeRawRelativeFrequencies(string input_path_source,
                                     string input_path_target);
