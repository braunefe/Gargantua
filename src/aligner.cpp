...
//makes sentence length alignment, returns correspondances with likelihood higer than 0.05 and traceback
pair<stack<Chunk>, vector<list<pair<int,int> > > > Aligner::makeLengthBasedAlignment(stack<Data>& data, const LengthDistributions& l_d)
{
	...
	vector<list<pair<int,int> > > all_diagonals; //vector to hold all flat diagonals
	stack<  pair<int, int> > trace_back;
	list< pair<int, int> > flat_diagonal;
	list< pair<int, int> > :: iterator itr_diag;
	pair<stack<Chunk>, vector<list<pair<int,int> > > > bests_and_diagonal;//beads with best scores and flattened diagonal

	//for debugging	
	//string path_to_log_file = "./alignment_log_files/Tracebacks_form_length.txt";
	...
	while(!data.empty())
	{
		...
		for(itr_diag = flat_diagonal.begin(); itr_diag != flat_diagonal.end(); itr_diag++)
		{
...
stack<Chunk>  Aligner :: makeInitialAlignment(stack<Data> data, const LengthDistributions &l_d, const LexicalDistributions &lex_d, vector<list<pair<int, int> > > f_d)
{
	//for debugging
...
//clusters the "initial alignment" (composed of 1-to-0/0-to-1 and 1-to-1 alignments) to obtain the final alignment (up to 4 sentences on one side)
stack<Chunk>  Aligner :: makeAlignmentClustering(stack<Chunk> sentence_alignment, LengthDistributions l_d, LexicalDistributions lex_d)
{
...
list<pair<int,int> > Aligner::flattenDiagonal(stack<pair<int,int> > &traceback_from_initial_alignment){
	...
	while(!traceback_from_initial_alignment.empty())
	{
		...
		else{next_cell.first = -1; next_cell.second = -1;}
		 
		//check for 1-to-1 alignment (next = current -1 on both sides)
		if(next_cell.first == current_cell.first-1 && next_cell.second == current_cell.second-1)
		{
			//do not modify
		...
		if(next_cell.first == current_cell.first-1 && next_cell.second == current_cell.second)
		{
			//do not modify
			flat_diagonal.push_front(current_cell);
		}
		...
		if(next_cell.first == current_cell.first && next_cell.second == current_cell.second-1)
		{
			//do not modify
			flat_diagonal.push_front(current_cell);
		}

		//check for 2-to-1 alignment (next = current -2 on source; next = current -1 on target)
		if(next_cell.first == current_cell.first -2 && next_cell.second == current_cell.second-1)
		{
		...
		//check for 1-to-2 alignment (next = current -1 on source; next = current -2 on target)
		if(next_cell.first == current_cell.first-1 && next_cell.second == current_cell.second-2)
		{
