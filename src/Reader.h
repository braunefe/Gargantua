...
class Reader {
 private:
  ...
  static void taggDocument(const char *path_source, const char *filename_source,
                           const char *path_target, const char *filename_target,
  ...
  static void numberDocument(const char *path_to_source,
                             ...
                             long int &target_counter);

 public:
  /*Methods to handle input documents*/

  // Taggs all Europarl documents
  static void makeIdTaggs(string path_source, string path_target,
                          string dest_source, string dest_target);

  // Numbers all Europarl documents
  static void numberData(string path_source, string path_target,
                         string dest_source, string dest_target);

  // Splits an Europarl document into paragraphs
  static stack<Data> splitEuroparlDocumentIntoParagraphs(const char *filename1,
                                                         const char *filename2);

  // Splits Europarl document into speakers
  static stack<Data> splitEuroparlDocumentIntoSpeakers(const char *filename1,
                                                       const char *filename2);

  // Splits Europarl document into chapters
  static stack<Data> splitEuroparlDocumentIntoChapters(const char *filename1,
                                                       const char *filename2);

  // splits all Europarl documents into paragraphs
  static void splitEuroparlDataIntoParagraphs(string source_path,
                                              string target_path);

  // reads Eruoparl documents and stores information in Data object
  static Data readEuroparlDocument(const char *filename_1,
                                   const char *filename_2);

  // reads numbered Europarl document and stores information in Data object
  static Data readNumberedEuroparlDocument(const char *filename_source,
                                           const char *filename_target);
  ...
  */
  ...
  static void makeTrainingData(string &path_to_best_alignments,
                               string &path_to_training_data);

  // removes words with occurrences fewer than X from training data and replaces
  // with the "other" word
  static pair<vector<long int>, vector<long int> >
  filterOutRareWordsFromTrainingData(
  ...
  static void filterOutRareWordsFromData(vector<long int> &rare_source_words,
  ...
  static void numberTrainingData(string &path_to_training_data,
                                 string &path_to_numbered_training_data);

  // makes the map indicating co-occurrence between source and target token
  static void makeCoOccurrenceMap(string &path_to_training_data,
