...
Chunk::Chunk(vector<long int> scp, vector<long int> tcp,
             vector<vector<long int> > sct, vector<vector<long int> > tck) {
...
vector<long int> Chunk::getSourceChunkPosition() { return sourceChunkPosition; }
...
vector<long int> Chunk::getTargetChunkPosition() { return targetChunkPosition; }
...
bool Chunk::comparePositions(Chunk c) {
  ...
  if ((c1s.size() != c2s.size()) || (c1t.size() != c2t.size())) {
    ...
    for (itrc1s = c1s.begin(), itrc2s = c2s.begin();
         itrc1s != c1s.end() && itrc2s != c2s.end(); itrc1s++, itrc2s++) {
      long int p1 = *itrc1s;
      long int p2 = *itrc2s;
    ...
    for (itrc1t = c1t.begin(), itrc2t = c2t.begin();
         itrc1t != c1t.end() && itrc2t != c2t.end(); itrc1t++, itrc2t++) {
      long int p1 = *itrc1t;
      long int p2 = *itrc2t;
